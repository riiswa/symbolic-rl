...
class Agent():
    def __init__(self, states, actions, lr = 0.01, gamma = 0.9, p = 1, epsilon_parameters = (0.5, 0.1, 0.1, 10)):
        ...
        self.Q_table = np.zeros((len(self.states), self.actions.n))
        #Hyperparameters :
        self.lr = lr 
        self.gamma = gamma 
    ...
    def update_Q_values(self, current_s, action, reward, next_s, done):
        # TODO : Use done parameter in next version
    ...
    def epsilon(self, time, nb_episodes):
        ...
        epsilon=1.1 -(1/cosh+(time*self.esp_param[2]/nb_episodes))
        return self.esp_param[3]*epsilon 

    # Afficher la fonction epsilon utilisée pour l'exploration/exploitation
    def print_espilon_function(self, nb_iterations):
        times =list(range(0,nb_iterations))
        epsilon_values=[self.epsilon(time, nb_iterations) for time in times]
    ...
    def chose_action(self, state, eps):
        if np.random.uniform(0, 1.1) > eps:
            print(f"Q-learning action : {np.argmax(self.Q_table[state])}  Eps : {eps}")
            return np.argmax(self.Q_table[state])
        else:
            action = self.actions.sample()
            print(f"Random action : {action}  Eps : {eps}")
            return action
...
agent.print_espilon_function(100)
"""
